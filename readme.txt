...
Usage: 
    ...
    To run the tool you use this command: bash staticAnalyzerTool.sh <options list> 
    ...
    Examples of invocation commands: 
        bash staticAnalyzerTool.sh -r ~/src                                     #scan the repository in home/src directory  
        bash staticAnalyzerTool.sh -l ~/src/file1.java ~/src2/example.py        #scan the two files file1.java and example.py  
        bash staticAnalyzerTool.sh -r ~/src -n srcScan                          #scans the repo and uses "srcScan" as the base name for reports 
        bash staticAnalyzerTool.sh -r ~/src -xt java                            #scan all files in repo besides the java ones (end in .java) 
        bash staticAnalyzerTool.sh -r ~/src -x                                  #scan all files in repo besides the ones indicated in /data/excludeFile.yaml
        bash staticAnalyzerTool.sh -r ~/src -v                                  #scan the repository and display verbose report on command line  
    ...
    The static analyzers used by this tool can be extended by adding the required infomation /data/internalFile.json. 
    data/internalFileNameConv.txt describes the schema that internalFile.json uses. Sonarqube and Pylint can serve as examples on how to 
    use the internalFile.json
